...
async function activate(context) {
  ...
  if (snippetsDir) {
    snippetsDir = import_coc10.workspace.expand(snippetsDir);
  ...
  if (configuration.get("autoTrigger", true)) {
    let inserting = false;
    import_coc10.events.on("TextInsert", async (bufnr, info) => {
  ...
  subscriptions.push(import_coc10.workspace.registerKeymap(["x"], "convert-snippet", async () => {
    ...
    if (!doc)
      return;
    let range = await import_coc10.window.getSelectedRange(mode);
