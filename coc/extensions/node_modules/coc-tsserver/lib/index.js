...
var require_cancellation = __commonJS({
  "node_modules/vscode-jsonrpc/lib/common/cancellation.js"(exports) {
    ...
    })(CancellationToken22 = exports.CancellationToken || (exports.CancellationToken = {}));
  ...
  "node_modules/semver/classes/range.js"(exports, module2) {
    var Range19 = class {
      constructor(range, options) {
        options = parseOptions(options);
        if (range instanceof Range19) {
          if (range.loose === !!options.loose && range.includePrerelease === !!options.includePrerelease) {
            return range;
          } else {
            return new Range19(range.raw, options);
          }
        }
    ...
    var Comparator = class {
      ...
      intersects(comp, options) {
        ...
        if (this.operator === "") {
          ...
          return new Range19(comp.value, options).test(this.value);
        } else if (comp.operator === "") {
          if (comp.value === "") {
            return true;
          }
          return new Range19(this.value, options).test(comp.semver);
        }
...
var require_satisfies = __commonJS({
  "node_modules/semver/functions/satisfies.js"(exports, module2) {
    ...
    var satisfies = (version, range, options) => {
      try {
        range = new Range19(range, options);
      } catch (er) {
        return false;
      }
...
var require_to_comparators = __commonJS({
  "node_modules/semver/ranges/to-comparators.js"(exports, module2) {
    var Range19 = require_range();
    var toComparators = (range, options) => new Range19(range, options).set.map((comp) => comp.map((c) => c.value).join(" ").trim().split(" "));
...
var require_max_satisfying = __commonJS({
  "node_modules/semver/ranges/max-satisfying.js"(exports, module2) {
    ...
    var maxSatisfying = (versions, range, options) => {
      ...
      let rangeObj = null;
      try {
        rangeObj = new Range19(range, options);
      } catch (er) {
        return null;
      }
      versions.forEach((v) => {
...
var require_valid2 = __commonJS({
  "node_modules/semver/ranges/valid.js"(exports, module2) {
    ...
    var validRange = (range, options) => {
      try {
        return new Range19(range, options).range || "*";
      } catch (er) {
        return null;
      }
    };
...
var require_intersects = __commonJS({
  "node_modules/semver/ranges/intersects.js"(exports, module2) {
    ...
    var intersects = (r1, r2, options) => {
      r1 = new Range19(r1, options);
      r2 = new Range19(r2, options);
...
var require_subset = __commonJS({
  "node_modules/semver/ranges/subset.js"(exports, module2) {
    ...
    var subset = (sub, dom, options = {}) => {
      if (sub === dom)
        return true;
      sub = new Range19(sub, options);
      dom = new Range19(dom, options);
...
((Range19) => {
  ...
  Range19.fromLocations = (start, end) => language.Range.create(Math.max(0, start.line - 1), Math.max(start.offset - 1, 0), Math.max(0, end.line - 1), Math.max(0, end.offset - 1));
  Range19.toFormattingRequestArgs = (file2, range) => ({
    file: file2,
  ...
  Range19.toFileRangeRequestArgs = (file2, range) => ({
    file: file2,
...
var FileConfigurationManager = class {
  ...
  async ensureConfigurationForDocument(document, token) {
    let opts = await import_coc4.workspace.getFormatOptions(document.uri);
  ...
  getPreferences(language2, uri) {
    ...
    const config = import_coc4.workspace.getConfiguration(`${language2}.preferences`, uri);
    ...
    }, getInlayHintsPreferences(language2));
    return preferences;
  }
...
function getInlayHintsPreferences(language2) {
  const config = import_coc4.workspace.getConfiguration(language2);
  return {
...
var TypeScriptInlayHintsProvider = class {
  constructor(client, fileConfigurationManager, languageIds) {
    this.client = client;
    this.fileConfigurationManager = fileConfigurationManager;
    this.languageIds = languageIds;
    this.inlayHintsNS = import_coc17.workspace.createNameSpace("tsserver-inlay-hint");
    this._disposables = [];
    this._tokenSource = void 0;
    this._inlayHints = /* @__PURE__ */ new Map();
    ...
    import_coc17.workspace.onDidChangeConfiguration(async (e) => {
      if (e.affectsConfiguration(section)) {
        for (let doc of import_coc17.workspace.documents) {
    ...
    import_coc17.workspace.onDidOpenTextDocument(async (e) => {
      const doc = import_coc17.workspace.getDocument(e.bufnr);
      await this.syncAndRenderHints(doc);
    }, null, this._disposables);
    import_coc17.workspace.onDidChangeTextDocument(async (e) => {
      const doc = import_coc17.workspace.getDocument(e.bufnr);
  ...
  dispose() {
    ...
    (0, import_coc17.disposeAll)(this._disposables);
    this._disposables = [];
    this._inlayHints.clear();
  }
  async renderHintsForAllDocuments() {
    for (let doc of import_coc17.workspace.documents) {
      await this.syncAndRenderHints(doc);
    }
  }
  async syncAndRenderHints(doc) {
    if (!this.languageIds.includes(doc.textDocument.languageId))
      return;
    if (!this.inlayHintsEnabled(this.languageIds[0]))
      return;
    if (this._tokenSource) {
      this._tokenSource.cancel();
      this._tokenSource.dispose();
    ...
    try {
      this._tokenSource = new import_coc17.CancellationTokenSource();
      const { token } = this._tokenSource;
      const range = import_coc17.Range.create(0, 0, doc.lineCount, doc.getline(doc.lineCount).length);
  ...
  async renderHints(doc, hints) {
    ...
    for (const item of hints) {
      const chunks = [[item.text, "CocHintSign"]];
  ...
  inlayHintsEnabled(language2) {
    const preferences = getInlayHintsPreferences(language2);
  ...
  async provideInlayHints(document, range, token) {
    ...
    return response.body.map((hint) => {
      return {
        text: hint.text,
        position: import_coc17.Position.create(hint.position.line - 1, hint.position.offset - 1),
...
var LanguageProvider = class {
  constructor(client, fileConfigurationManager, description, typingsStatus) {
    ...
    this.disposables = [];
    import_coc26.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
  ...
  configurationChanged() {
    const config = import_coc26.workspace.getConfiguration(this.id, null);
    this.client.diagnosticsManager.setEnableSuggestions(this.id, config.get(suggestionSetting, true));
  }
  dispose() {
    (0, import_coc26.disposeAll)(this.disposables);
  }
  ...
  registerProviders(client, typingsStatus) {
    ...
    this._register(import_coc26.languages.registerCompletionItemProvider(clientId, "TSC", languageIds, new TypeScriptCompletionItemProvider(client, typingsStatus, this.fileConfigurationManager, this.description.id), TypeScriptCompletionItemProvider.triggerCharacters));
    if (this.client.apiVersion.gte(API.v230)) {
      this._register(import_coc26.languages.registerCompletionItemProvider(`${this.description.id}-directive`, "TSC", languageIds, new DirectiveCommentCompletionProvider(client), ["@"]));
    }
    let definitionProvider = new TypeScriptDefinitionProvider(client);
    this._register(import_coc26.languages.registerDefinitionProvider(languageIds, definitionProvider));
    this._register(import_coc26.languages.registerTypeDefinitionProvider(languageIds, definitionProvider));
    this._register(import_coc26.languages.registerImplementationProvider(languageIds, definitionProvider));
    this._register(import_coc26.languages.registerReferencesProvider(languageIds, new TypeScriptReferences(client)));
    this._register(import_coc26.languages.registerHoverProvider(languageIds, new TypeScriptHoverProvider(client)));
    this._register(import_coc26.languages.registerDocumentHighlightProvider(languageIds, new TypeScriptDocumentHighlightProvider(this.client)));
    this._register(import_coc26.languages.registerSignatureHelpProvider(languageIds, new TypeScriptSignatureHelpProvider(client), ["(", ",", "<", ")"]));
    this._register(import_coc26.languages.registerDocumentSymbolProvider(languageIds, new TypeScriptDocumentSymbolProvider(client)));
    this._register(import_coc26.languages.registerRenameProvider(languageIds, new TypeScriptRenameProvider(client, this.fileConfigurationManager)));
    let formatProvider = new TypeScriptFormattingProvider(client, this.fileConfigurationManager);
    this._register(import_coc26.languages.registerDocumentFormatProvider(languageIds, formatProvider));
    this._register(import_coc26.languages.registerDocumentRangeFormatProvider(languageIds, formatProvider));
    this._register(import_coc26.languages.registerOnTypeFormattingEditProvider(languageIds, formatProvider, [";", "}", "\n", String.fromCharCode(27)]));
    this._register(import_coc26.languages.registerCodeActionProvider(languageIds, new InstallModuleProvider(client), "tsserver"));
    if (this.client.apiVersion.gte(API.v380) && typeof import_coc26.languages["registerCallHierarchyProvider"] === "function") {
      this._register(import_coc26.languages.registerCallHierarchyProvider(languageIds, new TypeScriptCallHierarchySupport(client)));
    }
    if (this.client.apiVersion.gte(API.v370)) {
      const provider = new TypeScriptDocumentSemanticTokensProvider(client);
      if (typeof import_coc26.languages["registerDocumentSemanticTokensProvider"] === "function") {
        this._register(import_coc26.languages.registerDocumentSemanticTokensProvider(languageIds, provider, provider.getLegend()));
      }
      if (typeof import_coc26.languages["registerDocumentRangeSemanticTokensProvider"] === "function") {
        this._register(import_coc26.languages.registerDocumentRangeSemanticTokensProvider(languageIds, provider, provider.getLegend()));
      }
    }
    ...
    if (this.client.apiVersion.gte(API.v280)) {
      this._register(import_coc26.languages.registerFoldingRangeProvider(languageIds, new TypeScriptFoldingProvider(this.client)));
      this._register(import_coc26.languages.registerCodeActionProvider(languageIds, new OrganizeImportsCodeActionProvider(this.client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.SourceOrganizeImports]));
    }
    if (this.client.apiVersion.gte(API.v240)) {
      this._register(import_coc26.languages.registerCodeActionProvider(languageIds, new TypeScriptRefactorProvider(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.Refactor]));
    }
    this._register(import_coc26.languages.registerCodeActionProvider(languageIds, new TypeScriptQuickFixProvider(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
    this._register(import_coc26.languages.registerCodeActionProvider(languageIds, new ImportFixProvider(this.client.bufferSyncSupport), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
    let cachedResponse = new CachedNavTreeResponse();
    if (this.client.apiVersion.gte(API.v206) && conf.get("referencesCodeLens.enable")) {
      this._register(import_coc26.languages.registerCodeLensProvider(languageIds, new TypeScriptReferencesCodeLensProvider(client, cachedResponse, this.description.id)));
    }
    if (this.client.apiVersion.gte(API.v220) && conf.get("implementationsCodeLens.enable")) {
      this._register(import_coc26.languages.registerCodeLensProvider(languageIds, new TypeScriptImplementationsCodeLensProvider(client, cachedResponse, this.description.id)));
    }
    if (this.client.apiVersion.gte(API.v350)) {
      this._register(import_coc26.languages.registerSelectionRangeProvider(languageIds, new SmartSelection(this.client)));
    }
    if (this.client.apiVersion.gte(API.v300)) {
      this._register(new TagClosing(this.client, this.description.id));
    }
    if (this.client.apiVersion.gte(API.v440) && import_coc26.workspace.isNvim) {
  ...
  handles(resource, doc) {
    ...
    return this.handlesConfigFile(import_coc26.Uri.parse(resource));
  }
  ...
  diagnosticsReceived(diagnosticsKind, file2, diagnostics) {
    const config = import_coc26.workspace.getConfiguration(this.id, file2.toString());
...
function defaultPathNormalizer(resource) {
  let u = import_coc27.Uri.parse(resource);
  if (u.scheme === "file") {
    return u.fsPath;
  }
...
var SyncedBufferMap = class extends ResourceMap {
  getForPath(filePath) {
    return this.get(import_coc28.Uri.file(filePath).toString());
  }
  get allBuffers() {
    return this.values;
  }
...
var BufferSyncSupport = class {
  constructor(client, modeIds) {
    ...
    import_coc28.workspace.onDidChangeConfiguration(this.updateConfiguration, this, this.disposables);
  }
  listen() {
    if (this.listening)
      return;
    this.listening = true;
    import_coc28.workspace.onDidOpenTextDocument(this.openTextDocument, this, this.disposables);
    import_coc28.workspace.onDidCloseTextDocument(this.onDidCloseTextDocument, this, this.disposables);
    import_coc28.workspace.onDidChangeTextDocument(this.onDidChangeTextDocument, this, this.disposables);
    import_coc28.workspace.textDocuments.forEach(this.openTextDocument, this);
  }
  handles(resource) {
  ...
  ensureHasBuffer(resource) {
    ...
    const existingDocument = import_coc28.workspace.textDocuments.find((doc) => doc.uri.toString() === resource);
    if (existingDocument) {
  ...
  openTextDocument(document) {
    ...
    const filepath = this.client.normalizePath(import_coc28.Uri.parse(resource));
    if (!filepath) {
      return false;
    }
    if (this.syncedBuffers.has(resource)) {
  ...
  sendPendingDiagnostics() {
    ...
    if (orderedFileSet.size) {
      let uris = Array.from(orderedFileSet.uris).map((uri) => import_coc28.Uri.parse(uri));
  ...
  updateConfiguration() {
    const jsConfig = import_coc28.workspace.getConfiguration("javascript", null);
    const tsConfig = import_coc28.workspace.getConfiguration("typescript", null);
...
var DiagnosticsManager = class {
  constructor() {
    ...
    this._currentDiagnostics = import_coc29.languages.createDiagnosticCollection("tsserver");
  }
  dispose() {
  ...
  diagnosticsReceived(kind, uri, diagnostics) {
    ...
    if (!collection)
      return;
    let doc = import_coc29.workspace.getDocument(uri);
    if (doc)
      uri = doc.uri;
    if (diagnostics.length === 0) {
...
var TypeScriptServiceConfiguration = class {
  constructor() {
    this._configuration = import_coc30.workspace.getConfiguration("tsserver");
    this._includePackageJsonAutoImports = import_coc30.workspace.getConfiguration("typescript").get("preferences.includePackageJsonAutoImports");
    import_coc30.workspace.onDidChangeConfiguration(() => {
      this._configuration = import_coc30.workspace.getConfiguration("tsserver");
      this._includePackageJsonAutoImports = import_coc30.workspace.getConfiguration("typescript").get("preferences.includePackageJsonAutoImports");
    });
  }
  ...
  get npmLocation() {
    ...
    if (path9)
      return import_coc30.workspace.expand(path9);
    try {
...
var Logger = class {
  get output() {
    ...
    this._channel = import_coc31.window.createOutputChannel("tsserver");
    return this._channel;
  }
  dispose() {
    if (this._channel) {
...
var Tracer = class {
  ...
  static readTrace() {
    let result = Trace.fromString(import_coc32.workspace.getConfiguration("tsserver").get("trace.server", "off"));
...
var TypeScriptVersionProvider = class {
  ...
  get globalVersion() {
    let { globalTsdk } = this.configuration;
    if (globalTsdk)
      return new TypeScriptVersion(import_coc33.workspace.expand(globalTsdk));
    return void 0;
  }
  ...
  getLocalVersion() {
    let folders = import_coc33.workspace.workspaceFolders.map((f) => import_coc33.Uri.parse(f.uri).fsPath);
  ...
  get bundledVersion() {
    try {
      ...
      import_coc33.window.showMessage("Bundled typescript module not found", "error");
      return null;
    }
  }
};
...
var VersionStatus = class {
  constructor(_normalizePath, enableJavascript) {
    ...
    this._versionString = "";
    this._versionBarEntry = import_coc34.window.createStatusBarItem(99);
    this._onChangeEditorSub = import_coc34.events.on("BufEnter", this.onBufEnter, this);
  ...
  async onBufEnter(bufnr) {
    let filetype = await import_coc34.workspace.nvim.call("getbufvar", [bufnr, "&filetype", ""]);
...
var TypeScriptServiceClient = class {
  constructor(pluginManager, modeIds, tscPathVim) {
    ...
    this.state = import_coc36.ServiceStat.Initial;
  ...
  restartTsServer() {
    if (this.tsServerProcess) {
      this.state = import_coc36.ServiceStat.Stopping;
  ...
  stop() {
    return new Promise((resolve) => {
      ...
      if (tsServerProcess && this.state == import_coc36.ServiceStat.Running) {
        this.info("Killing TS Server");
  ...
  startService(resendModels = false) {
    ...
    if (!currentVersion || !currentVersion.isValid) {
      if (this.configuration.globalTsdk) {
        import_coc36.window.showErrorMessage(`Can not find typescript module, in 'tsserver.tsdk': ${this.configuration.globalTsdk}`);
      } else {
        import_coc36.window.showErrorMessage(`Can not find typescript module, run ':CocInstall coc-tsserver' to fix it!`);
      }
      return;
    }
    this._apiVersion = currentVersion.version;
    this._tscPath = currentVersion.tscPath;
    import_coc36.workspace.nvim.setVar("Tsserver_path", this._tscPath, true);
  ...
  startProcess(currentVersion, args, options, resendModels) {
    ...
    this.state = import_coc36.ServiceStat.Starting;
    try {
      let childProcess = fork(currentVersion.tsServerPath, args, options, this.logger);
      this.state = import_coc36.ServiceStat.Running;
      ...
      handle.onError((err) => {
        if (this.token != myToken)
          return;
        import_coc36.window.showErrorMessage(`TypeScript language server exited with error. Error message is: ${err.message}`);
        this.error("TSServer errored with error.", err);
        this.error(`TSServer log file: ${this.tsServerLogFile || ""}`);
        import_coc36.window.showMessage(`TSServer errored with error. ${err.message}`, "error");
      ...
      this.state = import_coc36.ServiceStat.StartFailed;
  ...
  async openTsServerLogFile() {
    ...
    let echoErr = (msg) => {
      import_coc36.window.showErrorMessage(msg);
    };
    if (isRoot) {
    ...
    try {
      await import_coc36.workspace.nvim.command(`edit ${this.tsServerLogFile}`);
      return true;
    } catch {
  ...
  serviceExited(restart) {
    this.state = import_coc36.ServiceStat.Stopped;
    this.tsServerLogFile = null;
    ...
    if (restart) {
      ...
      if (this.numberRestarts > 5) {
        ...
        if (diff < 10 * 1e3) {
          this.lastStart = Date.now();
          startService = false;
          import_coc36.window.showMessage("The TypeScript language service died 5 times right after it got started.", "error");
        } else if (diff < 60 * 1e3) {
          this.lastStart = Date.now();
          import_coc36.window.showMessage("The TypeScript language service died unexpectedly 5 times in the last 5 Minutes.", "error");
        }
      }
      if (startService) {
        this.startService(true);
  ...
  toPath(uri) {
    return this.normalizePath(import_coc36.Uri.parse(uri));
  }
  ...
  toResource(filepath) {
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc36.Uri.parse(filepath);
    ...
    return import_coc36.Uri.file(filepath).toString();
  }
  ...
  getDocument(resource) {
    if (resource.startsWith("untitled:")) {
      ...
      return import_coc36.workspace.getDocument(bufnr);
    }
    return import_coc36.workspace.getDocument(resource);
  }
  ...
  asUrl(filepath) {
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc36.Uri.parse(filepath.slice(this.inMemoryResourcePrefix.length));
        if (this.inMemoryResourcePrefix) {
          const dirName = import_path8.default.dirname(resource.path);
          const fileName = import_path8.default.basename(resource.path);
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
    ...
    return import_coc36.Uri.file(filepath);
  }
  ...
  fatalError(command, error) {
    ...
    if (this.state === import_coc36.ServiceStat.Running) {
  ...
  dispatchEvent(event) {
    switch (event.event) {
      ...
      case "projectsUpdatedInBackground":
        ...
        const resources = body.openFiles.map(import_coc36.Uri.file);
    ...
    if (this.apiVersion.gte(API.v230)) {
      ...
      pluginPaths = pluginPaths.reduce((p, c) => {
        if (import_path8.default.isAbsolute(c)) {
          ...
          let roots = import_coc36.workspace.workspaceFolders.map((o) => import_coc36.Uri.parse(o.uri).fsPath);
  ...
  getProjectRootPath(uri) {
    let root = import_coc36.workspace.cwd;
    let u = import_coc36.Uri.parse(uri);
    if (u.scheme !== "file")
      return void 0;
    let folder = import_coc36.workspace.getWorkspaceFolder(uri);
    if (folder) {
      root = import_coc36.Uri.parse(folder.uri).fsPath;
    } else {
      let filepath = import_coc36.Uri.parse(uri).fsPath;
  ...
  cancelInflightRequestsForResource(resource) {
    if (this.state !== import_coc36.ServiceStat.Running || !this.tsServerProcess) {
      return;
    }
...
var standardLanguageDescriptions = [
  {
    ...
    modeIds: [typescript, typescriptreact, typescripttsx, typescriptjsx],
...
var AtaProgressReporter = class {
  constructor(client) {
    ...
    this._invalid = false;
    this.statusItem = import_coc37.window.createStatusBarItem(10, { progress: true });
    const disposables = [];
  ...
  async onTypesInstallerInitializationFailed() {
    this.statusItem.hide();
    if (!this._invalid) {
      const config = import_coc37.workspace.getConfiguration("typescript");
      if (config.get("check.npmIsInstalled", true)) {
        const dontShowAgain = "Don't Show Again";
        const selected = await import_coc37.window.showWarningMessage("Could not install typings files for JavaScript language features. Please ensure that NPM is installed or configure 'typescript.npm' in your user settings. visit https://go.microsoft.com/fwlink/?linkid=847635  to learn more.", dontShowAgain);
...
var TypeScriptServiceClientHost = class {
  constructor(descriptions, pluginManager, tscPath) {
    ...
    const configFileWatcher = import_coc38.workspace.createFileSystemWatcher("**/[tj]sconfig.json");
    ...
    const packageFileWatcher = import_coc38.workspace.createFileSystemWatcher("**/package.json");
    ...
    this.disposables.push(import_coc38.languages.registerWorkspaceSymbolProvider(new TypeScriptWorkspaceSymbolProvider(this.client, allModeIds)));
    this.client.onConfigDiagnosticsReceived((diag) => {
      let { body } = diag;
      if (body) {
        let { configFile, diagnostics } = body;
        let uri = import_coc38.Uri.file(configFile);
        if (diagnostics.length == 0) {
    ...
    import_coc38.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
    this.configurationChanged();
  }
  ...
  dispose() {
    (0, import_coc38.disposeAll)(this.disposables);
  ...
  configurationChanged(e) {
    if (!e || e.affectsConfiguration("tsserver")) {
      const config = import_coc38.workspace.getConfiguration("tsserver");
  ...
  async findLanguage(uri) {
    try {
      ...
      return languages4.find((language2) => language2.handlesUri(import_coc38.Uri.parse(uri)));
    } catch {
      return void 0;
    }
  }
...
var TsserverService = class {
  constructor(pluginManager, subscriptions) {
    ...
    this.id = "tsserver";
    this.name = "tsserver";
    this._state = import_coc39.ServiceStat.Initial;
    ...
    import_coc39.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("tsserver")) {
        const config2 = import_coc39.workspace.getConfiguration("tsserver");
  ...
  registCommands() {
    ...
    const registCommand2 = (cmd) => {
      let { id, execute } = cmd;
      subscriptions.push(import_coc39.commands.registerCommand(id, execute, cmd));
    };
  ...
  get config() {
    return import_coc39.workspace.getConfiguration("tsserver");
  }
  getClientHost() {
    if (this.state == import_coc39.ServiceStat.Running)
  ...
  async start() {
    if (!this.enable || this._state == import_coc39.ServiceStat.Starting)
      return;
    this._state = import_coc39.ServiceStat.Starting;
    if (this.clientHost) {
      let client2 = this.clientHost.serviceClient;
      client2.restartTsServer();
      return;
    }
    let tscPath = await import_coc39.workspace.nvim.getVar("Tsserver_path");
  ...
  async stop() {
    ...
    this._state = import_coc39.ServiceStat.Stopped;
  }
  dispose() {
    void this.stop();
...
var PluginManager = class {
  constructor() {
    ...
    import_coc40.extensions.onDidActiveExtension(loadPlugins, void 0, this._disposables);
    import_coc40.extensions.onDidUnloadExtension(loadPlugins, void 0, this._disposables);
  }
  dispose() {
  ...
  readPlugins() {
    ...
    for (const extension of import_coc40.extensions.all) {
...
async function activate(context) {
  ...
  subscriptions.push(import_coc41.services.regist(service));
  return {
